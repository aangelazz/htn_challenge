"""
This is the main file from which all endpoints to the API are defined.
"""
...
## Helper function gets current timestamp with millisecond precision
def get_exact_time():
    return datetime.now().strftime('%Y-%m-%dT%H:%M:%S.%f')

##The following are endpoints required for the challenge --------------------------------------
...
## Endpoint to create a new hacker (for uses in case one was not registered properly)
...
def create_hacker():
    ...
    ## return error if hacker already exists
    if existing_hacker:
        cursor.close()
        connection.close()
        ## returns error with HTTP status code 400 (bad request - invalid request data)
...
def delete_hacker(hacker_id):
    ...
    ## Execute the SQL query to update the updated_at before deletion (in case hacker information is somehow backed up)
...
def add_scan(hacker_id):
    ...
    ## Output error if no category/name given
    ...
    ## Return error if hacker not existing in database
    if not hacker:
        cursor.close()
        connection.close()
        ## returns error with HTTP status code 404 (not found - hacker not found)
...
def scan_data():
    ...
    ## Format SQL query with additional frequency parameters
    if frequencies:
    ...
    ## Format the frequencies and activity name for clarity
    formatted = [
